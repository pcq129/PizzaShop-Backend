...
class OrderController extends Controller
{
    ...
    public function search(Request $request)
    {
        try {
            ...
            // Handle status filter (including "0")
            ...
            $filtered_data = $orders->paginate($per_page);
    ...
    public function export_to_excel(Request $request)
    {
        ...
        try {
            ...
            // Handle status filter (0 when status is all or not selected)
            if (isset($status) && $status != '0') {
                $orders->where('order_status', $status);
            }
            ...
            $order_collection = $orders->get([
                'id', 'created_at', 'customer_id', 'payment_status',
                'payment_mode', 'rating', 'bill_amount'
            ])->map(function ($order) {
                return (object) [
                    'ID' => $order->id,
                    ...
                    'Rating' => optional(json_decode($order->rating))->food,
